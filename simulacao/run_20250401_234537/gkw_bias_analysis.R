...
rel_bias_plots <- lapply(unique_params, function(param) {
  ...
  # TeX(paste0("$\\", param, "$"), output = "character")
    labs(title = TeX(paste0("$\\", param, "$")))
})
...
cat("  Creating consolidated z bias plot...\n")

# Filter data by parameter and create separate plots
rel_bias_plots_z <- lapply(unique_params, function(param) {
  plot_bias_z(
    param_results %>% filter(parameter == param),
    param,
    title = NULL
...
# Combine all plots using patchwork
combined_rel_bias_z <- wrap_plots(rel_bias_plots_z, ncol = 2)

# Add a common legend
combined_rel_bias_z <- combined_rel_bias_z +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

# Save the combined plot
ggsave(file.path(plots_dir, "combined_rel_bias_z.pdf"),
       combined_rel_bias, width = 16, height = 12)
